...
def main():
    ...
    for i in range(start, end):
        ...
        for runstep in range(400): 
            ...
            modify_try = cv2.resize(modify_try.reshape(npop*3,32,32).transpose(1,2,0), dsize=(299, 299), interpolation=cv2.INTER_CUBIC).transpose(2,0,1).reshape(npop, 3,32,32)
            ...
            if runstep % 10 == 0:
                #modify_test = cv2.resize(modify[0].transpose(1,2,0), dsize=(299, 299), interpolation=cv2.INTER_CUBIC).transpose(0,3,1,2)
                modify_test = cv2.resize(modify.reshape(3,32,32).transpose(1,2,0), dsize=(299, 299), interpolation=cv2.INTER_CUBIC).transpose(2,0,1).reshape(1, 3,32,32)
