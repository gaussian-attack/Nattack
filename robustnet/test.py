...
def train_other(dataloader_test, net, loss_f, lr, name='adam', max_epoch=10):
    run_time = 0.0
    for epoch in range(max_epoch):
        beg = time.time()
        run_time += time.time() - beg
        print("[Epoch {}] Time: {}, Test loss: {}, Test accuracy: {}".format(epoch, run_time, loss(dataloader_test, net, loss_f), accuracy(dataloader_test, net)))
...
def main():
    ...
    for period in range(opt.epoch // 100):
        train_other(dataloader_test, net, loss_f, opt.lr, opt.method, 100)
        opt.lr /= 10
    # save model
