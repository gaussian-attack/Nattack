...
#                 realclipinput = realclipinput.transpose(0, 2, 3, 1)
                ...
                # print(outputsreal)
                if (np.argmax(outputsreal) != targets) and (np.abs(modify_test).max() <= epsi):
...
#                     imsave(folder+classes[targets[0]]+'_'+str("%06d" % batch_idx)+'.jpg',inputs.transpose(1,2,0))
            ...
            clipinput = (clipdist + newimg).reshape(npop,3,299,299)
