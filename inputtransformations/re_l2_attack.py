...
#                 reall2dist = np.sum((realinputimg - (np.tanh(newimg) * boxmul + boxplus))**2)**0.5
#                 realclipdist = np.clip(realdist, -epsi, epsi)
#                 realclipinput = realclipdist + (np.tanh(newimg) * boxmul + boxplus)
#                 realclipdist = realdist
#                 realclipinput = realclipdist + (np.tanh(newimg.numpy()) * boxmul + boxplus)
                realclipinput = realinputimg
#                 l2real =  np.mean((realclipinput - (np.tanh(newimg) * boxmul + boxplus))**2)**0.5
                l2real =  np.mean((realclipinput - inputs.transpose(2,0,1))**2)**0.5
                ...
                if (l2real >= epsi):
                    break
                if (np.argmax(outputsreal[0]) != targets) and (l2real <= epsi):
                    succImages += 1
                    success = True
...
#             l2dist = np.mean(np.square(inputimg - (np.tanh(newimg) * boxmul + boxplus)).reshape(npop,-1), axis = 1)**0.5
            l2dist = np.mean(np.square(inputimg - inputs.transpose(2,0,1)).reshape(npop,-1), axis = 1)**0.5

#                 l2dist[l2dist>=l2thresh] = l2thresh/(l2dist[l2dist>=l2thresh])
#                 l2dist[l2dist<l2thresh] = 1
                #print(l2dist[l2dist >= l2thresh].shape)
#                 dist = dist * ((l2thresh/l2dist).reshape(npop,1,1,1))
#                 clipinput = (dist + (np.tanh(newimg.numpy()) * boxmul + boxplus)).reshape(npop,3,32,32)
            clipinput = inputimg
            #l2distnew = np.sum(np.square(clipinput - (np.tanh(newimg) * boxmul + boxplus)).reshape(npop,-1), axis = 1)**0.5
